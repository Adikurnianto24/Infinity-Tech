<header>
    <nav class="flex_align _container">
        <a class="logo-link d-b" href="{{ route('home') }}"><img class="d-b" src="{{ asset('img/infinitylogo.png') }}" alt="logo"></a>
        <form class="ml-auto one-form" action="{{ route('shop') }}" method="GET">
            <input type="search" name="search" placeholder="Cari di toko kami..." value="{{ $search ?? '' }}">
            <button type="submit">
